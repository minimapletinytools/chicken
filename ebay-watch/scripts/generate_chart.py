...
<style>
  * { box-sizing: border-box; }
  ...
  #tags { display: flex; flex-wrap: wrap; align-items: baseline; gap: 0.25rem 1rem; }
  #tags label { cursor: pointer; }
  select, button {
    background: #222; color: #eee; border: 1px solid #444; padding: 3px 8px;
    border-radius: 4px; font: inherit;
  }
  button { cursor: pointer; }
  button:hover { background: #333; }
  #chart-row { display: flex; gap: 1.5rem; align-items: flex-start; width: 100%; }
  #chart-container { position: relative; flex: 1 1 auto; min-width: 0; height: 75vh; }
  #legend {
    display: flex; flex-direction: column; gap: 1px; max-height: 75vh; overflow-y: auto;
    min-width: 260px; max-width: 22vw; flex: 0 0 auto;
  }
  .legend-row {
...
<div id="controls">
  ...
  <div>
    <button id="selectAllTags" type="button">Select all</button>
    <button id="deselectAllTags" type="button">Unselect all</button>
  </div>
  <div id="tags"></div>
</div>
<div id="chart-row">
...
function setAllTags(checked) {
  tagsEl.querySelectorAll('input[type=checkbox]').forEach(cb => { cb.checked = checked; });
  render();
}
document.getElementById('selectAllTags').addEventListener('click', () => setAllTags(true));
document.getElementById('deselectAllTags').addEventListener('click', () => setAllTags(false));
